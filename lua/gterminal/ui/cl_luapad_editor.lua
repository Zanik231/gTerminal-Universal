...
function luapad.EditorPanel:SetName(name)
	self.TextEntry.name = name
  print("setname =",self.TextEntry.name)
end
 
function luapad.EditorPanel:SetEntity(entity)
	self.TextEntry.entity = entity
  print("setent =",self.TextEntry.entity)
end
...
function luapad.EditorPanel:SetCmds(arr)
  self.tasmCmds = arr
end 
function luapad.EditorPanel:SyntaxColorLine(row)
	...
	colors = {
		["none"] =  { Color(75, 255, 80,255), false},
		["number"] =    { Color(251, 96, 127, 255), false},
		["function"] =  { Color(100, 100, 255, 255), false},
		["enumeration"] =  { Color(255, 250, 50, 255), false},
		["metatable"] =  { Color(250, 210, 1, 255), false},
		["string"] =    { Color(120, 120, 120, 255), false},
		["expression"] =    { Color(255, 165, 0, 255), false},
		["operator"] =  { Color(64,224,208, 255), false},
    ["const"] =  { Color(255, 90, 90, 255), false},
		["comment"] =   { Color(230, 230, 240, 255), false},
    [","] =   { Color(192, 192, 192, 255), false},
	...
	while self.char do
      ...
      or sstr == "nil" or sstr == "NULL" or sstr == "and" or sstr == "not"
			or sstr == "or" or sstr == "||" or sstr == "&&" or luapad.CheckGlobal(sstr)=="func") then
				
				token = "expression"
      elseif(sstr == "Thrue" or sstr == "Flsea") then
        token = 'const'
        
      elseif(luapad.CheckGlobal(sstr)=="const") then
        token = "const"
 ...
 function luapad.EditorPanel:PaintLine(row)
	...
	draw.SimpleText(tostring(row), "LuapadEditor", width * 3, (row - self.Scroll[1]) * height, Color(250, 250, 250, 255), TEXT_ALIGN_RIGHT)
...
net.Receive("gTerminal.LuaPadEditor.Open", function()
	...
	local frame = vgui.Create( "DFrame" )
        ...
        frame:Center()
        frame:MakePopup()
        frame:SetTitle("LuapadEditor - " .. name)
        frame:SetDraggable(false)
	local textentry = vgui.Create("LuapadEditor", frame)
		textentry:Dock(FILL)
		textentry:DockPadding(0,0,0,-200)
		textentry:SetText(cont)
	...
	local save = vgui.Create("DButton", frame)
        ...
        save.DoClick = function(s)
            if #textentry:GetValue() > 32761 then
                local error_frame = vgui.Create( "DFrame", frame )
                error_frame:Center()
                ...
                error_frame:SetMouseInputEnabled(false)
                ...
                local close_error = frame:Add("DButton", error_frame)
			...
			local comp_text = util.Compress(textentry:GetValue())
